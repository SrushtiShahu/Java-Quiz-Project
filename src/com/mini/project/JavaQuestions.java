...
//			hs.add(5);
		...
		try {
			...
			PreparedStatement ps = con.prepareStatement(
					"select questions,choice1,choice2,choice3,choice4,main_ans from student_data " + "order by rand()");
...
//					System.out.println(q1);
		...
		try {
			...
			PreparedStatement pps = con
					.prepareStatement("insert into AllStudentDetails(student_name,student_marks) values(?,?)");
...
//			System.out.println();
		...
		try {
			...
			PreparedStatement psp = con
					.prepareStatement("select student_name, student_marks, student_grade from AllStudentDetails"
	...
	public void setStudentsGrade() {
		...
		try {
			...
			PreparedStatement psp = con.prepareStatement(
					"select student_id,student_marks,student_name,student_grade from AllStudentDetails");
			ResultSet rst = psp.executeQuery();

			while (rst.next()) {
//					System.out.println(rst.getString(3)+"\t"+rst.getInt(1)+"\t"+rst.getString(4));
				PreparedStatement pp = con
						.prepareStatement("update AllStudentDetails set student_grade = ? where student_id = ?");
